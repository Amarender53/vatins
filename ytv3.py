...
if selected_channel:
    ...
    # Sentiment Summary
    sentiment_summary = pd.Series(comments_df["sentiment"]).value_counts()
    sentiment_text = "\n".join([f"{s}: {c}" for s, c in sentiment_summary.items()])

    # Top Entities
    if entities_list:
        entity_df = pd.DataFrame(entities_list, columns=["Entity", "Type"])
        top_entities = entity_df.value_counts().reset_index(name="Count").head(10)
        entity_text = "\n".join([f"{row['Entity']} ({row['Type']}): {row['Count']}" for _, row in top_entities.iterrows()])
    else:
    ...
    # Add all to PDF
    pdf.chapter("📄 Top 10 Translated Comments", top_comments)
    pdf.chapter("📊 Sentiment Summary", sentiment_text)
    pdf.chapter("🧠 Detected Topics", topics_text)
    pdf.chapter("🔎 Top Named Entities", entity_text)
    ...
    st.download_button("📥 Download PDF Summary", pdf_buffer, file_name="youtube_ai_summary.pdf", mime="application/pdf")
